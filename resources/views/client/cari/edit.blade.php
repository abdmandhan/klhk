...
@section('content')
        ...
        <div class="row">
        <div class="col-lg-12">
        <form action="/front/cari/{{$cari->id}}/update" method="POST">  
            {{csrf_field()}}       
        ...
        <div class="form-group">
            ...
            <!-- <input name="eselon" input type="text " class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Eselon"
            value="{{$cari->eselon}}"> -->
            <select name="eselon" class="form-control" id="exampleFormControlSelect1">
            @foreach($eselon as $s)
                <option value="{{$s->id}}" @if($cari->id_eselon == $s->id) selected @endif>{{$s->name}}</option>
            @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            <option value="2" @if($cari->kategori == 2) selected @endif>Website</option>
            <option value="1"@if($cari->kategori == 1) selected @endif>Sistem Informasi</option>
            
            </select>
        </div>
        <div class="form-group">
            <label for="exampleFormControlTextarea1">Deskripsi</label>
            <textarea name="deskripsi" class="form-control" id="exampleFormControlTextarea1" rows="3">{{$cari->description}}</textarea>
         </div>
         <div class="form-group">
            <label for="exampleFormControlSelect1">Pilih Status</label>
            <select name = "status" class="form-control" id="exampleFormControlSelect1">
            <option value="1" @if($cari->status == 1) selected @endif>Aktif</option>
            <option value="0"@if($cari->status == 0) selected @endif>Tidak Aktif</option>
            <option value="2"@if($cari->status == 2) selected @endif>Ubuntu/option>
        <div class="form-group">
